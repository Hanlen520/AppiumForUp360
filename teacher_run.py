...
from teacher.case.TeacherLogin import TeacherLoginCase
from teacher.case.TeacherNotice import TeacherNoticeCase
from teacher.case.TeacherHomework import TeacherHomeworkCase
...
def get_driver(i):
    ...
    driver = base_driver.android_driver(i, "Teacher")
    return driver
...
def create_suite(i):
    ...
    data = case_list.get_value('T', 'case_list_'+str(i))
    if len(data) > 0:
        for case in data:
            methods = case['method']
...
def get_suite(i):
    ...
    file_path = os.getcwd() + "\\report\\report\\" + day + "\\teacher"
    ...
    runner = HTMLTestRunner.HTMLTestRunner(stream=fp, title=u'向上网老师端自动化测试报告', description=u'用例执行情况:')
    runner.run(suite)
    fp.close()
    email = Email()
...
if __name__ == '__main__':
    ...
    # full_path = config_dir + "//pages//yaml//teacher//"
    # event_handler = ScriptEventHandler("teacher")
    # observer = Observer()
